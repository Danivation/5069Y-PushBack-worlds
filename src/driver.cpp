...
void IntakeControl() {
    while (true) {
        if (master.get_digital_new_press(INTAKE)) {
            intake.move(127);
        }
        if (master.get_digital_new_press(OUTTAKE)) {
            intake.move(-127);
        }
...
void LiftControl() {
    while (true) {
        if (master.get_digital_new_press(LIFT_UP)) {
            lift.move(127);
        }
        if (master.get_digital_new_press(LIFT_DOWN)) {
            lift.move(-127);
        }
