...
void auton_left_split() {
    ...
    store();
    c_lemlib.moveToPoint(-1.1_tiles, -0.84_tiles, 1000, {.minSpeed = 10, .earlyExitRange = 2}, true);
    delay(100);
    loader.extend();

    // js get in the goal somehow ig
    c_lemlib.moveToPoint(-1.8_tiles, -1_tiles, 700, {.forwards = false, .minSpeed = 95, .earlyExitRange = 4}, false);
    c_lemlib.swingToHeading(168, lemlib::DriveSide::LEFT, 250, {}, false);
    ...
    score();
    int score1Start = millis();
    waitUntilCondition(millis() >= score1Start + 650);
    c_lemlib.cancelMotion();
    lemlibDistReset({&right_beam});
    ...
    loader.retract();
    wing.retract();
    c_lemlib.moveToPoint(-2_tiles+0.55_tiles, -1.6_tiles, 1100, {.minSpeed = 10, .earlyExitRange = 2}, true);
    unjam();
    c_lemlib.waitUntilDone();
    ...
    left_mg.set_brake_mode_all(MotorBrake::hold);
    left_mg.set_brake_mode_all(MotorBrake::hold);
    c_lemlib.moveToPoint(-2_tiles+0.4_tiles, -0.88_tiles, 1000, {.forwards = false, .minSpeed = 80, .earlyExitRange = 5}, false);
...
void auton_right_split() {
    ...
    // drive into loader
    c_lemlib.moveToPoint(1.98_tiles, -56, 1300, {.maxSpeed = 90, .minSpeed = 35, .earlyExitRange = 6.5}, false);
    store();
    int loader1Start = millis();
    c_lemlib.moveToPoint(1.98_tiles, -70, 1000, {.maxSpeed = 45}, true);
...
void auton_left_4ball_loader() {
    ...
    int loader1Start = millis();
    c_lemlib.moveToPoint(-1.99_tiles, -70, 1000, {.maxSpeed = 45}, true);
    waitUntilCondition(millis() >= loader1Start + 700);
    c_lemlib.cancelMotion();
...
void auton_right_4ball_stack() {
    ...
    // js get in the goal somehow ig
    c_lemlib.moveToPoint(1.8_tiles, -1_tiles, 700, {.forwards = false, .minSpeed = 95, .earlyExitRange = 4}, false);
    c_lemlib.swingToHeading(-160, lemlib::DriveSide::RIGHT, 250, {}, false);
...
void auton_left_7ball() {
    ...
    c_lemlib.moveToPoint(-1.75_tiles, -1.7_tiles, 1300, {.minSpeed = 70, .earlyExitRange = 6}, false);
...
void auton_left_7ball_counter() {
    ...
    left_mg.set_brake_mode_all(MotorBrake::hold);
    left_mg.set_brake_mode_all(MotorBrake::hold);
    c_lemlib.moveToPoint(-2_tiles+0.44_tiles, -1.05_tiles, 1000, {.forwards = false, .minSpeed = 70, .earlyExitRange = 7}, false);
    c_lemlib.moveToPoint(-2_tiles+0.4_tiles, -10, 1000, {.forwards = false}, false);
...
void auton_right_7ball_counter() {
    ...
    // sweep out position
    store();
    c_lemlib.moveToPoint(2_tiles-0.6_tiles, -1.7_tiles, 1100, {.forwards = false, .minSpeed = 10, .earlyExitRange = 2}, false);
    intake_raise.retract();
    c_lemlib.turnToHeading(0, 400, {}, false);
    lemlibDistReset({&right_beam});

    c_lemlib.moveToPoint(2_tiles-0.45_tiles, -1_tiles, 1000, {.forwards = true, .minSpeed = 80, .earlyExitRange = 5}, false);
    c_lemlib.moveToPoint(2_tiles-0.42_tiles, -7, 1000, {.forwards = true, .maxSpeed = 90}, false);
...
void auton_skills() {
    ...
    // drive ALMOST to end
    c_lemlib.moveToPoint(16, 64, 1200, {.maxSpeed = 120, .minSpeed = 55, .earlyExitRange = 4}, true);
